...
export function QuizResult({
  ...
  const isDebugMode = typeof window !== 'undefined' && window.location.search.includes('debug=result');
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans antialiased relative overflow-hidden pb-16">
      ...
      <div className="w-full max-w-4xl mx-auto px-4 pt-12 relative z-10">
        ...
        {/* --- ESTADO 1: RESULTADO GRATUITO (SEMPRE EXIBIDO PRIMEIRO) --- */}
        <div className="text-center mb-8 animate-fade-in">
          ...
          <p className="text-xs md:text-sm uppercase tracking-widest text-slate-400 font-bold mb-2">
          ...
          <div className="max-w-2xl mx-auto bg-slate-900/60 backdrop-blur-xl border border-slate-800 rounded-2xl p-5 mb-6 text-left">
            <p className="text-slate-200 text-sm md:text-base leading-relaxed font-medium">
              {shortSummary}
            </p>
          </div>
          
          <p className="text-xs md:text-sm text-slate-500 italic font-medium max-w-xl mx-auto border-t border-slate-900 pt-4 mb-6">
            "Pela primeira vez, o sistema olhou para você — não apenas para a sua pesquisa."
          </p>

          {/* DADOS E INTENSIDADE */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-2xl mx-auto text-left mb-6">
            <div className="bg-slate-900/60 backdrop-blur-xl border border-slate-800 rounded-2xl p-5 flex flex-col justify-center items-center text-center">
              <span className="text-xs font-semibold uppercase tracking-wider text-slate-500 mb-1">Sua Pontuação</span>
              <div className="text-3xl font-extrabold text-white flex items-baseline gap-1">
                {dominant.score} <span className="text-sm text-slate-500 font-medium">/ 12</span>
              </div>
              <span className="text-[11px] text-slate-500 mt-1">Fator de correspondência comportamental</span>
            </div>

            <div className="bg-slate-900/60 backdrop-blur-xl border border-slate-800 rounded-2xl p-5 flex flex-col justify-center items-center text-center">
              <span className="text-xs font-semibold uppercase tracking-wider text-slate-500 mb-1">Grau de Intensidade</span>
              <div className={`px-3 py-1.5 rounded-xl text-base font-bold border ${getIntensityColor(dominant.intensity)}`}>
                {dominant.intensity}
              </div>
              <span className="text-[11px] text-slate-500 mt-1">Nível de incidência na sua rotina acadêmica</span>
            </div>
          </div>
        </div>

        {/* --- FORMULÁRIO DE LIBERAÇÃO (EXIBIDO QUANDO NÃO DESBLOQUEADO) --- */}
        {!unlocked && (
          <div className="bg-slate-900/80 backdrop-blur-xl border-2 border-violet-500/30 rounded-3xl p-6 md:p-10 mb-12 text-center shadow-[0_0_35px_rgba(139,92,246,0.15)] animate-fade-in">
            <div className="w-14 h-14 rounded-2xl bg-violet-500/10 border border-violet-500/30 flex items-center justify-center text-3xl mx-auto mb-4">
              🔒
            </div>

            <h2 className="text-xl md:text-2xl font-extrabold text-white mb-3 leading-snug">
        ...
        {/* --- CONTEÚDO BLOQUEADO (RENDERIZADO SOMENTE APÓS DESBLOQUEIO) --- */}
        {unlocked && (
          <div className="animate-fade-in">
            {/* Confirmation Banner */}
            <div className="bg-emerald-500/10 border border-emerald-500/30 rounded-2xl p-4 mb-8 text-center text-emerald-400 font-bold text-sm flex items-center justify-center gap-2">
              <span>✅</span> Diagnóstico liberado com sucesso.
            </div>

            <div ref={diagnosisRef} id="diagnosis-start" className="scroll-mt-6" />
            ...
            {/* --- DOBRA 6: CONTRASTE DE POSICIONAMENTO --- */}
            <div className="bg-slate-900/60 backdrop-blur-xl border border-slate-800 rounded-3xl p-6 md:p-10 mb-8">
            ...
            {/* --- DOBRA 7: A TRANSIÇÃO METACOGNITIVA --- */}
