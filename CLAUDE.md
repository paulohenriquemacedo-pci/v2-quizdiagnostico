...
- `supabase/functions/` — admin Edge Functions: `get-quiz-responses`, `update-quiz-response`, `delete-quiz-response`, `reset-quiz-data`, `get-funnel-metrics`. Plus one tracking-relay function that deliberately does **not** follow the admin pattern below: `capi-relay` (called by the browser, default JWT verification, no admin-role check). See Tracking above.
...
- `META_CAPI_ACCESS_TOKEN` is an Edge Function secret only (`supabase secrets set`) — never put it in `.env`, client code, or a `VITE_`-prefixed var. `META_PIXEL_ID` is also set as an Edge Function secret (separate from the client's `VITE_META_PIXEL_ID`, same value) since Deno Edge Functions don't read Vite's `.env`.
