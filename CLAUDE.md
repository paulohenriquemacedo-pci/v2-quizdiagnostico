...
**The click is a real, native `<a>` navigation — `CheckoutCTA` deliberately never calls `preventDefault()`.** An earlier version of this flow intercepted the click and redirected via `window.location.href` (reasoning: defend against injected third-party scripts breaking the click chain). That's worth avoiding by default: it silently breaks any click-based third-party tracking that relies on detecting a genuine browser-native click-through to the checkout domain (this project hit exactly that with LowTrack's pixel — see Tracking below). If click-based third-party tracking is added in the future and needs to survive a page unload, prefer `navigator.sendBeacon` for the first-party insert over intercepting the click.

### Tracking

Current state (2026-08-14): **no third-party tracking is installed** — `index.html` has no third-party `<script>` blocks. `src/lib/attribution.ts` (first-party, local-only UTM/fbclid capture) is the only tracking-adjacent code active on the client.

**What was tried and removed, in order, this session:** a Meta Pixel base snippet (added 2026-08-13 for an initial test, removed same day) → the **LowTrack** pixel (`lt_px_...`, loads `https://lowtrack.com.br/pixel.js`, added 2026-08-13). LowTrack's checkout CTA never produced a confirmed `InitiateCheckout` event despite the `CheckoutCTA` native-navigation fix above (which was a real, worthwhile fix regardless — the site's click behavior was verified correct byte-for-byte in production). Whether IC's detection is client-side (click-based, needing this repo's cooperation) or entirely server-side (computed by LowTrack from a Hotmart→LowTrack webhook this repo has no visibility into) was never conclusively determined — `lowtrack.com.br` blocks automated fetches (403), so their actual mechanism couldn't be verified from here, and debugging it consumed a lot of back-and-forth without resolution before LowTrack was removed entirely (2026-08-14) at the user's request. Before all of that, **all prior third-party tracking was deliberately removed on 2026-08-11** — a Meta Pixel, the Meta Conversions API relay (`capi-relay` Edge Function), and both Utmify `<script>` blocks — as a full reset before rebuilding from scratch; search git history for `capi-relay`, `metaPixel.ts`, "Utmify Pixel Script" if you need the prior implementation's shape (a fairly complete Meta Pixel + CAPI dual-fire setup with Advanced Matching, verified working at EMQ 6–9/10, torn out intentionally, not because it was broken).

**Before installing whatever comes next**, get a straight answer from the platform's own docs/support on how it detects checkout-adjacent events (client-side click listener vs. server-side webhook from the checkout provider) — that single question is what stalled the LowTrack integration. This repo controls the client-side click behavior (`CheckoutCTA`, verified correct); it has zero visibility into a tracking platform's own webhook config with the checkout provider.
...
- **Third-party event wiring:** `fireThirdPartyEvent()` in `src/lib/events.ts` is a no-op stub, and no third-party platform is installed at all right now (see Tracking above for why LowTrack didn't work out). Before picking the next one, get a straight answer on its event-detection model (client-side click vs. server-side webhook) — then wire a real call into that one function.
